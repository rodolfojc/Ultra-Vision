...
public class FindMemberModel {
	...
	// FIND MEMBER AND CREATES AN CUSTOMER/MEMBER OBJECT THAT IS PASS TO MAIN MENU INSTANCE
	// RETURN A CUSTOMER/MEMBER
	public Customer findId(int id) {
		...
		try {
			...
			DebitCreditCard tempCard = new DebitCreditCard(dataStr[5], dataStr[6], dataStr[7], dataStr[8], dataStr[9]);

			MembershipCard tempMemb = new MembershipCard(tempCard, dataInt[0], false);

			if (dataStr[0].equals("MusicLovers")) {

				this.tempMember = new MusicLovers(dataStr[1], dataStr[2], dataStr[3], dataStr[4], tempMemb, tempCard);

			} else if (dataStr[0].equals("VideoLovers")) {

				this.tempMember = new VideoLovers(dataStr[1], dataStr[2], dataStr[3], dataStr[4], tempMemb, tempCard);

			} else if (dataStr[0].equals("TvLovers")) {

				this.tempMember = new TvLovers(dataStr[1], dataStr[2], dataStr[3], dataStr[4], tempMemb, tempCard);

			} else {

				this.tempMember = new PremiunLovers(dataStr[1], dataStr[2], dataStr[3], dataStr[4], tempMemb, tempCard);
